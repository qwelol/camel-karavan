...
export function DslConnections() {
    ...
    const [width, height, top, left] = useDesignerStore((s) => [s.width, s.height, s.top, s.left], shallow);
    const [steps, addStep, deleteStep, clearSteps] = useConnectionsStore(
        (s) => [s.steps, s.addStep, s.deleteStep, s.clearSteps],
        shallow,
    );
    ...
    useEffect(() => {
        const toDelete1: string[] = Array.from(steps.keys()).filter(
            (k) => CamelDefinitionApiExt.findElementInIntegration(integration, k) === undefined,
        );
        ...
        if (pos) {
            ...
            return !isInternal ? (
                <g key={pos.step.uuid + '-incoming'}>
                    ...
                    />
                    <path
                        key={pos.step.uuid + '-path'}
                        d={`M ${lineX1},${lineY1} C ${lineX1},${lineY2} ${lineX2},${lineY1}  ${lineX2},${lineY2}`}
    ...
    function getIncomingIcons(data: [string, number, 'internal' | 'remote' | 'nav' | 'poll']) {
        ...
        if (pos) {
            ...
            const internalCall: boolean =
                step && uri && step?.dslName === 'FromDefinition' && INTERNAL_COMPONENTS.includes(uri);
    ...
    function addGap(
        data: [string, number, 'internal' | 'remote' | 'nav' | 'poll'][],
    ): [string, number, 'internal' | 'remote' | 'nav' | 'poll'][] {
        ...
        let outs: [string, number, 'internal' | 'remote' | 'nav' | 'poll'][] = Array.from(steps.values())
            ...
            .filter(
                (pos) =>
                    ...
                    (pos.step.dslName === 'KameletDefinition' && !CamelUi.isActionKamelet(pos.step)),
            )
            .filter(
                (pos) =>
                    ['ToDefinition', 'PollDefinition'].includes(pos.step.dslName) && !CamelUi.isActionKamelet(pos.step),
            )
            ...
            return !isInternal ? (
                <g key={pos.step.uuid + '-outgoing'}>
                    ...
                    <path
                        ...
                        className={className}
                        markerStart={isPoll ? 'url(#arrowheadLeft)' : 'none'}
                        markerEnd={isPoll ? 'none' : 'url(#arrowheadRight)'}
                    />
                </g>
            ) : (
    ...
    function getOutgoingIcons(data: [string, number, 'internal' | 'remote' | 'nav' | 'poll']) {
        ...
        if (pos) {
            ...
            const name = internalCall
                ? step?.parameters?.name
                    ? step?.parameters.name
                    : step?.parameters?.address
                : '';
            const r = pos.headerRect.height / 2;
            const outgoingX = width - 20;
            const outgoingY = data[1] + RADIUS;
    ...
    function isSpecial(pos: DslPosition): boolean {
        ...
        ].includes(pos.step.dslName);
    }

    function addArrowToList(
        list: JSX.Element[],
        from?: DslPosition,
        to?: DslPosition,
        fromHeader?: boolean,
    ...
    function getArrow(pos: DslPosition): JSX.Element[] {
        ...
        if (pos.parent && pos.parent.dslName === 'TryDefinition' && pos.position === 0) {
            ...
            pos.parent &&
            ['RouteConfigurationDefinition', 'MulticastDefinition', 'LoadBalanceDefinition'].includes(
                pos.parent.dslName,
            )
        ) {
            const parent = steps.get(pos.parent.uuid);
            ...
            pos.parent &&
            ['WhenDefinition', 'OtherwiseDefinition', 'CatchDefinition', 'FinallyDefinition', 'TryDefinition'].includes(
                pos.parent.dslName,
            )
        ) {
            if (pos.position === 0) {
        ...
        if (
            ['WhenDefinition', 'OtherwiseDefinition'].includes(pos.step.dslName) &&
            pos.step.hasSteps() &&
            ...
            if (
                parentDsl &&
                ['RouteConfigurationDefinition', 'MulticastDefinition', 'LoadBalanceDefinition'].includes(parentDsl)
            ) {
                // do nothing
    ...
    function getComplexArrow(key: string, rect1: DOMRect, rect2: DOMRect, toHeader: boolean) {
        ...
        return <path key={uuidv4()} name={key} d={path} className='path' markerEnd='url(#arrowheadRight)' />;
    }
