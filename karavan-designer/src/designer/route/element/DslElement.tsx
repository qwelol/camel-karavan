...
export function DslElement(props: Props) {
    ...
    function sendPosition(el: HTMLDivElement | null) {
        ...
        if (el) {
            ...
            if (header) {
                const headerIcon: any = Array.from(header.childNodes.values()).filter((n: any) =>
                    n.classList.contains('header-icon'),
                )[0];
                const headerRect = headerIcon.getBoundingClientRect();
                const rect = el.getBoundingClientRect();
                if (step.showChildren) {
                    EventBus.sendPosition(
                        'add',
                        step,
                        prevStep,
                        nextStep,
    ...
    function getChildElements() {
        ...
        const notOnlySteps =
            children.filter((c) => c.name === 'steps').length === 1 &&
        ...
        if (step.dslName === 'CatchDefinition') {
            // exception
    ...
    function getAddStepButton() {
        const { step } = props;
        const hideAddButton =
            step.dslName === 'StepDefinition' &&
            !CamelDisplayUtil.isStepDefinitionExpanded(integration, step.uuid, selectedUuids.at(0));
        if (hideAddButton) return <></>;
        else
            return (
                <Tooltip position={'left'} content={<div>{'Add step to ' + CamelDisplayUtil.getTitle(step)}</div>}>
                    <button
                        type='button'
                        aria-label='Add'
                        onClick={(e) => onOpenSelector(e)}
                        className={
                            isAddStepButtonLeft() ? 'add-button add-button-left' : 'add-button add-button-bottom'
                        }
                    >
                        <AddElementIcon />
                    </button>
