...
export function useRouteDesignerHook() {
    ...
    const [
        ...
        (s) => [
            s.setParentId,
            s.setShowSelector,
            s.setSelectorTabIndex,
            s.setParentDsl,
            s.setShowSteps,
            s.setSelectedPosition,
            s.routeId,
            s.setRouteId,
        ],
        shallow,
        ...
        if (ce.dslName === 'FromDefinition') {
            ...
            for (let i = 0; i < flows.length; i++) {
                if (flows[i].dslName === 'RouteDefinition') {
                    const routeDefinition: RouteDefinition = flows[i];
    ...
    const selectElement = (element: CamelElement) => {
        ...
        const selectedElement = shiftKeyPressed
            ? CamelDefinitionApiExt.findElementInIntegration(integration, uuid)
            : element;
    ...
    function setDslDefaults(step: CamelElement): CamelElement {
        ...
        if (step.dslName === 'SplitDefinition') {
            ...
            if (
                split.expression?.groovy !== undefined &&
    ...
    function integrationImageDownload(printerRef: React.MutableRefObject<HTMLDivElement | null>) {
        ...
        if (ref !== null) {
            toPng(ref, {
                ...
                cacheBust: true,
                filter: integrationImageDownloadFilter,
                height: height,
                width: width,
