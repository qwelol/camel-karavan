...
export function DslSelector(_props: Props) {
    ...
    function setAllElements() {
        ...
        const cE = CamelUi.getSelectorModelsForParentFiltered(parentDsl, 'component', showSteps).filter(
            (dsl) => !blockedComponents.includes(dsl.uri || dsl.name),
        );
        const kE = CamelUi.getSelectorModelsForParentFiltered(parentDsl, 'kamelet', showSteps).filter(
            (dsl) => !blockedKamelets.includes(dsl.name),
        );
                ...
                <ToggleGroupItem
                    text={
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            ...
                            {ready && (
                                <Badge isRead={!isComp} className={isComp ? 'label-component' : ''}>
                                    {cCount}
                                </Badge>
                            )}
                        </div>
                ...
                <ToggleGroupItem
                    text={
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            ...
                            {ready && (
                                <Badge isRead={!isKam} className={isKam ? 'label-kamelet' : ''}>
                                    {kCount}
                                </Badge>
                            )}
                        </div>
