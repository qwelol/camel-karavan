...
export class CamelUi {
    ...
    static getSelectorModelTypes = (
        parentDsl: string | undefined,
        showSteps: boolean = true,
        filter: string | undefined = undefined,
    ): [string, number][] => {
        const navs = CamelUi.getSelectorModelsForParent(parentDsl, showSteps)
            .map((dsl) => dsl.navigation.split(','))
            .reduce((accumulator, value) => accumulator.concat(value), [])
            .filter((nav, i, arr) => arr.findIndex((l) => l === nav) === i)
            .filter((nav, _i, _arr) => !['dataformat'].includes(nav));
        const connectorNavs = [INTEGRATION_PATTERNS, 'kamelet', 'component'];
        const eipLabels = connectorNavs.filter((n) => navs.includes(n));
        return eipLabels.map((label) => [
            label,
            this.getSelectorModelsForParentFiltered(parentDsl, label, true).filter((dsl: DslMetaModel) =>
                filter === undefined ? true : CamelUi.checkFilter(dsl, filter),
            ).length,
        ]);
    };
    ...
    static dslHasSteps = (className: string): boolean => {
        return (
            CamelDefinitionApiExt.getElementChildrenDefinition(className).filter((c) => c.name === 'steps').length === 1
        );
    };
    ...
    static getSelectorRestMethodModels = (): DslMetaModel[] => {
        return [
            ...
            'DeleteDefinition',
            'HeadDefinition',
        ].map((method) => this.getDslMetaModel(method));
    };

    static getSelectorModelsForParentFiltered = (
        parentDsl: string | undefined,
        navigation: string,
        showSteps: boolean = true,
    ): DslMetaModel[] => {
        const models = CamelUi.getSelectorModelsForParent(parentDsl, showSteps).filter((dsl) =>
            dsl.navigation.includes(navigation),
        );
        return models;
    };
    ...
    static getSelectorModelsForParent = (parentDsl: string | undefined, showSteps: boolean = true): DslMetaModel[] => {
        ...
        if (!parentDsl) {
            ...
            if (showSteps) {
                if (
                    parentDsl &&
                    CamelDefinitionApiExt.getElementChildrenDefinition(parentDsl).filter(
                        (child) => child.name === 'steps',
                    ).length > 0
                ) {
                ...
                const children = CamelDefinitionApiExt.getElementChildrenDefinition(parentDsl).filter(
                    (child) => child.name !== 'steps',
                );
                children
                    .filter((child) => {
    ...
    static getComponentsDslMetaModel = (type: 'consumer' | 'producer'): DslMetaModel[] => {
        return ComponentApi.getComponents()
            .filter((c) => (type === 'consumer' ? !c.component.producerOnly : !c.component.consumerOnly))
            .map(
                (c) =>
                    new DslMetaModel({
    ...
    static javaNameFromTitle = (title: string): string => {
        ...
        return name
            .split('-')
            .map((v) => CamelUtil.capitalizeName(v))
            .join('');
    };
    ...
    static getInternalRouteUris = (
        integration: Integration,
        componentName: string,
        showComponentName: boolean = true,
    ): string[] => {
        const result: string[] = [];
        integration.spec.flows
            ?.filter((f) => f.dslName === 'RouteDefinition')
    ...
    static getElementTitle = (element: CamelElement): string => {
        if (element.dslName === 'RouteDefinition') {
            ...
            ['ToDefinition', 'ToDynamicDefinition', 'PollDefinition', 'FromDefinition', 'KameletDefinition'].includes(
                element.dslName,
            ) &&
            (element as any).uri
        ) {
            ...
            const kameletTitle =
                uri && uri.startsWith('kamelet:') ? KameletApi.findKameletByUri(uri)?.title() : undefined;
            return kameletTitle
                ? kameletTitle
    ...
    static isShowExpressionTooltip = (element: CamelElement): boolean => {
        if (Object.prototype.hasOwnProperty.call(element, 'expression')) {
    ...
    static getIconForDsl = (dsl: DslMetaModel): React.JSX.Element => {
        ...
        } else if (dsl.dsl && dsl.dsl === 'FromDefinition' && dsl.uri?.startsWith('kamelet')) {
    ...
    static getIconForElement = (element: CamelElement): JSX.Element => {
        ...
        if (['FromDefinition', 'KameletDefinition'].includes(element.dslName) && k !== undefined) {
            ...
            component !== undefined &&
            component.component.remote !== true
        ) {
    ...
    static getFlowCounts = (i: Integration): Map<string, number> => {
        ...
        result.set(
            'routeConfiguration',
            i.spec.flows?.filter((e: any) => e.dslName === 'RouteConfigurationDefinition').length || 0,
        );
    ...
    static getRoutes = (integration: Integration): CamelElement[] => {
        ...
        integration.spec.flows?.filter((e: any) => e.dslName === 'RouteDefinition').forEach((f: any) => result.push(f));
        return result;
    };
    ...
    static getRests = (integration: Integration): CamelElement[] => {
        const result: CamelElement[] = [];
        integration.spec.flows?.filter((e: any) => e.dslName === 'RestDefinition').forEach((f: any) => result.push(f));
        return result;
    };
    ...
    static getRouteConfigurations = (integration: Integration): RouteConfigurationDefinition[] | undefined => {
        const result: CamelElement[] = [];
        integration.spec.flows
            ?.filter((e: any) => e.dslName === 'RouteConfigurationDefinition')
