...
export const ExpressionVariables: ExpressionContext[] = [
    ...
    new ExpressionContext('simple', [
        ...
        new Context(
            'variable.foo.*OGNL* ',
            'refer to the foo variable and invoke its value using a Camel OGNL expression.',
        ),
        ...
        new Context(
            'messageAs(_type_).*OGNL* ',
            'Converts the message to the given type determined by its classname and then invoke methods using a Camel OGNL expression. The converted message can be null. ',
        ),
        ...
        new Context(
            'date:_command_',
            `evaluates to a Date object. Supported commands are: *now* for current timestamp,
...
Command accepts offsets such as: *now-24h* or *header.xxx+1h* or even *now+1h30m-100*.`,
        ...
        new Context(
            ...
            `Invoking a bean expression using the xref:components::bean-component.adoc[Bean] language.'),
...
to a bean class (such as calling a static method) then you can prefix with type, such as 'bean:type:fqnClassName'. `,
        ...
        new Context(
            'routeGroup',
            'Returns the route group of the current route the Exchange is being routed. Not all routes has a group assigned, so this may be null.',
        ),
        ...
        new Context(
            'empty(type)',
            `Creates a new empty object of the type given as parameter. The type-parameter-Strings are case-insensitive. +
'string' -> empty String
...
'map'    -> empty HashMap `,
        ...
        new Context(
            'collate(group)',
            `The collate function iterates the message body and groups
...
similar to the collate method in Groovy.`,
        ...
        new Context(
            ...
            `The join function iterates the message body (by default) and joins the data into a string. The separator is by default a comma. The prefix is optional.'),
...
source (simple language) such as a header via the exp parameter. For example join('&','id=','$\{header.ids}')`,
        ...
        new Context(
            ...
            'As messageHistory but without the exchange details (only includes the route stack-trace). This can be used if you do not want to log sensitive data from the message itself.',
        ),
        new Context(
            'uuid(type)',
            'Returns an UUID using the Camel `UuidGenerator`. You can choose between `default`, `classic`, `short` and `simple` as the type. If no type is given the default is used. It is also possible to use a custom `UuidGenerator` and bind the bean to the xref:manual::registry.adoc[Registry] with an id. For example `${uuid(myGenerator}` where the ID is _myGenerator_.',
        ),
        new Context(
            'hash(exp,algorithm)',
            'Returns a hashed value (string in hex decimal) using JDK MessageDigest. The algorithm can be SHA-256 (default) or SHA3-256.',
        ),
        new Context(
            'jsonpath(exp)',
            'When working with JSon data, then this allows to use the JsonPath language for example to extract data from the message body (in JSon format). This requires having camel-jsonpath JAR on the classpath.',
        ),
        new Context(
            'jsonpath(input,exp)',
            'When working with JSon data, then this allows to use the JsonPath language for example to extract data from the message body (in JSon format). This requires having camel-jsonpath JAR on the classpath. For _input_ you can choose `header:key`, `exchangeProperty:key` or `variable:key` to use as input for the JSon payload instead of the message body.',
        ),
        new Context(
            'jq(exp)',
            'When working with JSon data, then this allows to use the JQ language for example to extract data from the message body (in JSon format). This requires having camel-jq JAR on the classpath.',
        ),
        new Context(
            'jq(input,exp)',
            'When working with JSon data, then this allows to use the JQ language for example to extract data from the message body (in JSon format). This requires having camel-jq JAR on the classpath. For _input_ you can choose `header:key`, `exchangeProperty:key` or `variable:key` to use as input for the JSon payload instead of the message body.',
        ),
        new Context(
            'xpath(exp)',
            'When working with XML data, then this allows to use the XPath language for example to extract data from the message body (in XML format). This requires having camel-xpath JAR on the classpath.',
        ),
        new Context(
            'xpath(input,exp)',
        ...
        new Context(
            'pretty(exp)',
            'Converts the inlined expression to a String, and attempts to pretty print if JSon or XML, otherwise the expression is returned as the String value.',
        ),
    ]),
];
