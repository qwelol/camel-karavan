...
export const KnowledgebasePage = (props: Props) => {
    ...
    if (customOnly)
        kameletList = kameletList.filter((k) => KameletApi.getCustomKameletNames().includes(k.metadata.name));

    const components = ComponentApi.getComponents()
        .filter((c) => {
            return (
                c.component.name.toLowerCase().includes(filter.toLowerCase()) ||
    ...
    const elements = CamelModelMetadata.filter((c) => c.name.toLowerCase().includes(filter.toLowerCase())).sort(
        (a: ElementMeta, b: ElementMeta) => (a.name > b.name ? 1 : -1),
    );
                            ...
                            <Tab
                                ...
                                title={
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        Components
                                        <Badge className='label-component'>{components.length}</Badge>
                                    </div>
                                }
                            />
                            <Tab
                                ...
                                title={
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        ...
                                        <Badge className='label-eip'>{elements.length}</Badge>
                                    </div>
                                }
                            />
                            <Tab
                                ...
                                title={
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                        Kamelets
                                        <Badge className='label-kamelet'>{kameletList.length}</Badge>
                                    </div>
                                }
                            />
                        </Tabs>
