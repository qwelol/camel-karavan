...
import { CardHeader, Card, CardTitle, CardBody, CardFooter, Badge, Checkbox, Flex, Text } from '@patternfly/react-core';
...
export function ComponentCard(props: Props) {
    const [setComponent, setModalOpen, showBlockCheckbox] = useKnowledgebaseStore(
        (s) => [s.setComponent, s.setModalOpen, s.showBlockCheckbox],
        shallow,
    );
    ...
    const isBlockedComponent = blockedComponents
        ? blockedComponents.findIndex((r) => r === component.component.name) > -1
        : false;
    ...
    return (
        <Card isCompact key={component.component.name} className='knowledgebase-card' onClick={(event) => click(event)}>
            <CardHeader className='header-labels'>
                ...
                >
                    ...
                    <Badge isRead className='support-level labels'>
                        {component.component.supportLevel}
                    </Badge>
                </Flex>
                {showBlockCheckbox && (
                    <Checkbox
