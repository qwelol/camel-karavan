...
import { CardHeader, Card, CardTitle, CardBody, CardFooter, Badge, Checkbox, Flex } from '@patternfly/react-core';
...
export function KameletCard(props: Props) {
    const [setKamelet, setModalOpen, showBlockCheckbox] = useKnowledgebaseStore(
        (s) => [s.setKamelet, s.setModalOpen, s.showBlockCheckbox],
        shallow,
    );
    ...
    const isblockedKamelet = blockedKamelets
        ? blockedKamelets.findIndex((r) => r === kamelet.metadata.name) > -1
        : false;
    ...
    return (
        <Card isCompact key={kamelet.metadata.name} className='knowledgebase-card' onClick={(event) => click(event)}>
            <CardHeader className='header-labels'>
                ...
                {showBlockCheckbox && (
                    <Checkbox
                        id={kamelet.metadata.name}
                        className='block-checkbox labels'
                        isChecked={!isblockedKamelet}
